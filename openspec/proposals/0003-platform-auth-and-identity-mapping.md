...
2. **用户身份映射**：底座如何识别“这次请求代表哪个用户”，从而正确执行 `owner_id == sub` 的权限过滤。
...
3. 底座以 `sub` 作为 `owner_id`，执行权限过滤与审计。
...
1. **MVP 主路径选择方案 A**：平台透传终端用户 JWT，底座以 `sub` 执行权限过滤与审计。
...
- 审计日志能回答：谁（JWT `sub`）通过哪个平台入口在什么时间访问了哪个文档/片段。
...
- JWT `sub` 采用“内部用户 ID”还是“外部平台用户 ID”？若采用外部 ID，是否需要映射表与迁移策略？
